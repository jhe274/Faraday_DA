    ...
    def plot_process(self, x, y1, y2, run, xlabel, ylabel_y1, ylabel_y2, file_name):
        ...
        for i in self.number_of_runs(run):
            if i == run-1:
                ...
                ax1.plot(x[i]/time_factor, y1[i], color='C0', alpha=0.6, label=fr'$\dot{{B_z}}$={round(fitted_k*1e3*time_factor,1):.1f} mG/min')
                ...
                # Plot the magnetic field measurements
                ax1.plot(x[i]/time_factor, y1[i], color='C3', alpha=0.4, label=fr'$\dot{{B_z}}$={round(fitted_k*1e3*time_factor,1):.1f} mG/min')
...
if __name__ == "__main__":
    ...
    # for i in range(5,8):
    plotter.gaussmter_vs_time(gaussmeter_path, 11)