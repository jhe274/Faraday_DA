...
class Plot:
    ...
    def Ellipticity_vs_Frequency(self, lambda_path, lockin_path, run, n, B, power):
        ...
        for i in range(run-1, run+1):
            ...
            epsilon0, theta0 = [], []  # Initialize epsilon and theta lists inside the loop

            for j in range(len(x0[i-run+1])):
                epsilon0.append(y1[i-run+1][j] / (2 * np.pi * scipy.special.jv(1, 2.405) * y3[i-run+1][j]))                                # Ellipticity: [millirad]
                theta0.append(y2[i-run+1][j] * 1e3/ (2 * np.pi * scipy.special.jv(2, 2.405) * y3[i-run+1][j] * np.sqrt(1 - 4 * epsilon0[j]**2)))       # Rotation: [millirad]

            x.append(self.consts.c / x0[i-run+1] * 1e-9 - self.consts.Nu39_D2 * 1e-9)                                                           # [GHz]
            # ax.plot(x[i-run+1], epsilon0 * 1e3, label=r'$\epsilon_7$' if i-run+1==0 else r'$\epsilon_8$')
        ...
        """
        y6, y7, y8, y9, y10, epsilon1, theta1 = [], [], [], [], [], [], []
        for xval in x0[1]:
            ...
            if 0 <= cidx < len(x0[0]):
                ...
                y1_diff = y1[0][cidx] - y1[1][cidx]
                y2_diff = y2[0][cidx] - y2[1][cidx]
                y3_diff = y3[0][cidx] - y3[1][cidx]
                ep_diff = y4[0][cidx] - y4[1][cidx]
                th_diff = y5[0][cidx] - y5[1][cidx]
                y6.append(y1_diff)
                y7.append(y2_diff)
                y8.append(y3_diff)
                y9.append(ep_diff * 1e3)
                y10.append(th_diff * 1e3)
            else:
        ...
        for k in range(len(x0[2])):
            epsilon1.append(y6[k] / ( 2 * np.pi * scipy.special.jv(1,2.405) * y8[k]))                                                  # Ellipticity: [rad]
            theta1.append(y7[k] * 1e3/ (2 * np.pi * scipy.special.jv(2,2.405) * y8[k] * np.sqrt(1 - 4 * epsilon1[k]**2)))                          # Rotation: [rad]

        # ax.plot(x0[2], epsilon1 * 1e3, label=r'$R_7$-$R_8$')
        # ax.plot(x0[2], y9 * 1e3, label=r'$\epsilon_7$-$\epsilon_8$')
        ax.plot(x0[2], theta1, label=r'$R_7$-$R_8$')
        ax.plot(x0[2], y10, label=r'$\theta_7$-$\theta_8$')
